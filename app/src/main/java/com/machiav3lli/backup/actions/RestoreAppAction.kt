...
open class RestoreAppAction(context: Context, work: AppActionWork?, shell: ShellHandler) :
    ...
    protected fun openArchiveFile(
        ...
        if (compressionType == "zst") {
            inputStream = ZstdCompressorInputStream(inputStream)
        }
    ...
    fun genericRestoreFromArchiveTarApi(
        ...
        try {
            TarArchiveInputStream(
                openArchiveFile(archive, compressionType, isEncrypted, iv)
            ).use { archiveStream ->
    ...
    fun genericRestoreFromArchiveTarCmd(
        ...
        RootFile(targetPath).let { targetDir ->
            ...
            try {
                openArchiveFile(archive, compressionType, isEncrypted, iv).use { archiveStream ->
